...
2026-09-14 기준. 설계 근거는 [PRD.md](PRD.md), 사용법은 [README.md](README.md).
...
| 항목 | 상태 |
|---|---|
| 파이프라인 5단계 | ✅ 전 구간 동작. 실행 1회 ~10초 (326~361건 수집) |
| CLI (`run.py`) | ✅ 동작 |
| GUI (`app.py` + `web/`) | ✅ 동작. http://127.0.0.1:8765 |
| 테스트 | ✅ 63개 전부 통과 |
...
**현재 설정**: `lookback 24h` / `threshold 0.4` / `max_articles 8` / 소스 15개 전부 활성
...
output/                     <run_id>.md · <run_id>.json (실행 기록 11건 있음)
